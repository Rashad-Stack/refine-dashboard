...
export default function DashboardLatestActivities() {
  const {
    data: audits,
  ...
  const { data: deals, isLoading: dealsIsLoading } = useList({
    resource: "deals",
  ...
  const isLoading = auditIsLoading || dealsIsLoading;
                ...
                <List.Item.Meta
                  ...
                  description={
                    <Space size={4}>
                      ...
                      <Text>Deal</Text>
                      <Text>{deal?.action === "CREATED" ? "in" : "to"}</Text>
